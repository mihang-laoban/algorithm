...
/*给你一个链表，每index个节点一组进行翻转，请你返回翻转后的链表。
index是一个正整数，它的值小于或等于链表的长度。
如果节点总数不是index的整数倍，那么请将最后剩余的节点保持原有顺序。

示例：
给你这个链表：1->2->3->4->5
当index= 2 时，应当返回: 2->1->4->3->5
当index= 3 时，应当返回: 3->2->1->4->5
...
func TestSortList(t *testing.T) {
	head1 := ArrayToLinkedList([]int{4, 2, 1, 3, 5})
...
val：一个表示RandomListNode.val的整数。
random_index：随机指针指向的节点索引（范围从0到n-1）；如果不指向任何节点，则为nil。
...
/*给定一个链表，返回链表开始入环的第一个节点。如果链表无环，则返回nil。
...
/*给定一个链表，旋转链表，将链表每个节点向右移动index个位置，其中index是非负数。
