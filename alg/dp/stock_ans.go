...
func ImprovedStock(prices []int) {
	...
	for i := 1; i < size; i++ {
		dp[i&1][0][0] = 0
		dp[i&1][0][1] = Max(dp[(i-1)&1][1][0]+prices[i], dp[(i-1)&1][0][1])
		dp[i&1][0][2] = Max(dp[(i-1)&1][1][1]+prices[i], dp[(i-1)&1][0][2])
		dp[i&1][1][0] = Max(dp[(i-1)&1][0][0]-prices[i], dp[(i-1)&1][1][0])
		dp[i&1][1][1] = Max(dp[(i-1)&1][0][1]-prices[i], dp[(i-1)&1][1][1])
		dp[i&1][1][2] = 0
	}
	res := Max(dp[(size-1)&1][0][1], dp[(size-1)&1][0][2])
	fmt.Println(res)
}
